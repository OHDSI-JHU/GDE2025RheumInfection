...
# Define the cohort IDs for Aim 2
cohortIds <- c(
  1794548, 1794547, 1794546, 1794545, 1794544, 1794543,
  1794542, 1794541, 1794540, 1794539, 1794538, 1794537,
  1794536, 1794535, 1794534, 1794533, 1794532, 1794531,
  1794530, 1794529, 1794395, 1793813, 1794528, 1794527
)

# Define the indication IDs
indicationIds <- list(
  SLE = 1794240,
  Uveitis = 1794239,
  SSc = 1794241,
...
# Create combined excluded covariates list from all existing excluded covariate sets
# This assumes you have these objects defined in your environment
excludedCovariatesList <- list(
  excludedCovariates_JAKi_RTX,
  excludedCovariates_RTX_MMF,
  excludedCovariates_RTX_IVIG,
  excludedCovariates_JAKi_MMF,
  excludedCovariates_JAKi_IVIG,
  excludedCovariates_MMF_IVIG
)

# Combine all excluded covariate concept IDs into one comprehensive list
allExcludedConceptIds <- unique(unlist(lapply(excludedCovariatesList, function(x) {
...
# Remove any NA or NULL values
allExcludedConceptIds <- allExcludedConceptIds[!is.na(allExcludedConceptIds)]


# Generate all unique pairwise combinations for each indication
tcis <- list()

for (indicationName in names(indicationIds)) {
  indicationId <- indicationIds[[indicationName]]
  
  # Generate all unique pairs (no duplicates)
  for (i in 1:(length(cohortIds) - 1)) {
    for (j in (i + 1):length(cohortIds)) {
      targetId <- cohortIds[i]
      comparatorId <- cohortIds[j]
      
      # Create the target-comparator-indication combination
      # Using the combined excluded covariates for ALL comparisons
      tci <- list(
        targetId = targetId,
        ...
        minAge = NULL,
        maxAge = NULL,
        excludedCovariateConceptIds = allExcludedConceptIds  # Use combined list
      )
      
      tcis <- append(tcis, list(tci))
    }
  }
}

# Print summary statistics
print(sprintf("Total TCIs generated: %d", length(tcis)))
print(sprintf("TCIs per indication: %d", length(tcis) / length(indicationIds)))
print(sprintf("Unique cohort pairs: %d", choose(length(cohortIds), 2)))
print(sprintf("Using %d excluded covariate concept IDs for all comparisons", length(allExcludedConceptIds)))
